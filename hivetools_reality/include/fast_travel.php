...
$P1 = $db->query("SELECT `unique_id`, `name` FROM `profile` WHERE `unique_id` = '".$PLAYER."' OR `name`= '".$PLAYER."' ");
while ($row = mysqli_fetch_object($P1)){
$PLAYER_UID   = $row->unique_id;
$PLAYER_NAME = $row->name;
}

$P2 = $db->query("SELECT `worldspace` FROM `survivor` WHERE `unique_id` = '".$PLAYER_UID."' AND `world_id`='".$DZ_MAP."' AND `is_dead`='0'");
if (mysqli_num_rows($P2) == TRUE ) {
$db->query("UPDATE `survivor` SET `worldspace` = '[0,[".$TRAVEL."]]' WHERE `unique_id` = '".$PLAYER."' AND `world_id`='".$DZ_MAP."' AND `is_dead`='0' ");
echo'
<div class="ui-widget">
...
$P2 = $db->query("SELECT `worldspace` FROM `survivor` WHERE `unique_id` = '".$PLAYER_UID."' AND `world_id`='".$DZ_MAP."' AND `is_dead`='0'");
if (mysqli_num_rows($P2) == TRUE ) {
$db->query("UPDATE `survivor` SET `worldspace` = '".$select_iv."' WHERE `unique_id` = '".$PLAYER."' AND `world_id`='".$DZ_MAP."' AND `is_dead`='0' ");
echo'
<div class="ui-widget">
<div class="ui-state-highlight ui-corner-all" style="padding: 1px;">
<p style="text-align: center; font: 13px Tahoma;">
...
$select_p .= "\r\n <option  name='select_player' value='{$row['unique_id']}'>{$row['name']} </option>" ;
}
$select_p .= "\r\n</select>";
...
$select_iv.='<option name="select_iv" value="'.$row['worldspace'].'" />'.$row['class_name'].'</option>';
}
$select_iv.= '</select>';
