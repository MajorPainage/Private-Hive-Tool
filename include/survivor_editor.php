...
$profile = $db->query("SELECT * FROM `profile` WHERE `unique_id` = '".$player."' OR `name`= '".$player."' ");
...
<strong># ERROR! PLAYER "'.$_GET['player'].'" DOES NOT EXIST # </strong> 
...
$survivor = $db->query("SELECT * FROM `survivor` WHERE `unique_id` = '".$SUID."' AND `world_id`='".$DZ_MAP."' ");
...
$db->query("UPDATE `survivor` SET `is_dead`= '0' WHERE `id`='".$SID."' AND `unique_id`='".$SUID."' AND `world_id`='".$DZ_MAP."' AND `is_dead`='1' ");
$SDEAD=TRUE;
echo'
<div class="ui-widget">
...
$totent=$db->query("SELECT `worldspace` FROM `instance_deployable` WHERE `owner_id` = '".$SID."' AND `deployable_id`= '1' ");
if (mysqli_num_rows($totent) == TRUE ) {
while($row = mysqli_fetch_object($totent)) {
$TENT_SPACE = $row->worldspace;
}

$db->query("UPDATE `survivor` SET `worldspace`='".$TENT_SPACE."' WHERE `id`='".$SID."' AND `unique_id`='".$SUID."' AND `world_id`='".$DZ_MAP."' ");
...
$db->query("UPDATE `survivor` SET `medical`='".$SHEAL."' WHERE `unique_id`='".$SUID."' AND `world_id`='".$DZ_MAP."' AND `is_dead`= '0' ");
...
if (in_array($SMODEL, $modelsArray) && !empty($DZ_MAP) && !empty($SUID)){
...
$db->query("UPDATE `profile` SET `humanity`='".$SHUMAN."' WHERE `unique_id`='".$SUID."' ");
}



$db->query("UPDATE `survivor` SET `model`='".$SMODEL."', `inventory`='".$SINVENTORY."', `backpack`='".$SBACK_NEW."' WHERE `unique_id`='".$SUID."' AND `world_id`='".$DZ_MAP."' AND `is_dead`= '0' ");
...
$db->query("DELETE FROM id USING `instance_deployable` id INNER JOIN `survivor` s ON id.owner_id = s.id WHERE s.unique_id = '".$SUID."' AND s.world_id = '".$DZ_MAP."'");
$db->query("DELETE FROM `survivor` WHERE `unique_id` = '".$SUID."' AND `world_id`='".$DZ_MAP."' ");
$db->query("DELETE FROM `profile` WHERE `unique_id` = '".$SUID."' ");
...
href="'.$_SERVER['SCRIPT_NAME'].'?go='.$GETGO.'&map='.$GETMAP.'&player='.$SUID.'&model='.$SMODEL.'&sback='.$sbacks.'">'.$sbacks.''.$info_back.'</a></li>';
}
$select_SBACK .='</ul></div>';
echo $select_SBACK;
...
href="'.$_SERVER['SCRIPT_NAME'].'?go='.$GETGO.'&map='.$GETMAP.'&player='.$SUID.'&sback='.$SBACK.'&model='.$MODEL.'">'.$MODEL.'</a></li>';
}
$select_MODEL .='</ul></div>';
echo $select_MODEL; 
}
...
href="'.$_SERVER['SCRIPT_NAME'].'?go='.$GETGO.'&player='.$row['unique_id'].'">'.$row['name'].'</a></li>';
}
$select_p .= '</ul></div>';
...
<table border="0" class="border2"  cellpadding="0" cellspacing="0" width="99%">
  <form method="GET">	  
            <tr>
               ...
               <td bgcolor="#F2F2F2"></br> <input class="tab" type="button"  border="0" value="Select" data-dropdown="#player" />
	<br> 
