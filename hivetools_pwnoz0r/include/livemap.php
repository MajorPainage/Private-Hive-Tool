...
$Y       = array (10,15,100);
   ...
   $vehicle_result = $db->query("SELECT `Classname`, `ObjectID`, `Worldspace` FROM `object_data` 
   WHERE 
...
foreach($arr['user'] as $key => $ob){
       $x = ($ob['Worldspace']['x']/$X[0])+$X[1];
       $y = ($ob['Worldspace']['y']/$Y[0])-$Y[1];
       $mapX = round($ob['Worldspace']['x']/$X[2]);
       $mapY = round($ob['Worldspace']['y']/$Y[2]);
       $player .= ' 
	   <script>
   $(function() {
   $( "#'.$ob['PlayerUID'].'" ).dialog({
